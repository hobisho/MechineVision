...
def compute_disparity_and_remove_bg(img_path, num_disparities=96, block_size=11, disparity_threshold=30,name=""):
    ...
    # 構建一個3x3的核
    disparity_norm = cv2.medianBlur(disparity_norm, 3)  # 中值濾波器，核大小5
    ...
    # 進行侵蝕操作
    # disparity_norm = cv2.erode(disparity_norm, kernel, iterations=5)
    
    disparity_norm = cv2.dilate(disparity_norm, kernel, iterations=7)
    ...
    foreground[~mask] = [0, 0, 100]  # 設為黑色背景，這裡仍然保留了 RGB 格式
...
if __name__ == "__main__":
    name ="A"
    compute_disparity_and_remove_bg(f"{name}.jpg", num_disparities=16*2, block_size=5, disparity_threshold=50,name=f"./final/output/")
