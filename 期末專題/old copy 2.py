...
def shift_points_in_stack(depth_stack, color_stack, midcenter_x, midcenter_z, threshold=0.3, constant=1,time = 1):
    ...
    new_depth_stack = [[[] for _ in range(w + int(midcenter_x))] for _ in range(h)]
    new_color_stack = [[[] for _ in range(w + int(midcenter_x))] for _ in range(h)]
