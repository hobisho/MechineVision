...
def ImgShift(img1, img2):
    ...
    for x in range(width-2):
        col1 = img1[0:int(height/2) , width-3 : width-1]
    ...
    background_shift= width - best_x
    return background_shift, best_error

if __name__ == "__main__":
    path = "./final/image/bbox"
    img1 = cv2.imread(f"{path}_left_left.jpg")
    img2 = cv2.imread(f"{path}_right_right.jpg")
    print(img1.shape)

    pos, error = ImgShift(img1, img2)
    if pos >= 0:
