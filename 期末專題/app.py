...
def shift_points_in_stack_numpy(depth_stack, color_stack,
    ...
    new_depth_stack = np.full((h, w + int(midcenter_x), c * 3), np.nan, dtype=np.float32)
    new_color_stack = np.zeros((h, w + int(midcenter_x), c * 3, 3), dtype=np.uint8)
    ...
    # 4) 濾掉超出 [0, w) 的那部分
    inside = (new_x >= 0) & (new_x < w + int(midcenter_x))
...
if __name__ == "__main__":
    ...
    fast_left_shifted_depth, fast_left_shifted_color = shift_points_in_stack_numpy(fast_left_stack_depth, fast_left_stack_color, midcenter_x, midcenter_z, threshold=0.5, constant=-1)
    fast_right_shifted_depth, fast_right_shifted_color = shift_points_in_stack_numpy(fast_right_stack_depth, fast_right_stack_color, midcenter_x, midcenter_z, threshold=0.5, constant=1)
    ...
    fast_merged_depth_stack, fast_merged_color_stack = shift_points_in_stack_numpy(fast_merged_depth_stack, fast_merged_color_stack, midcenter_x, midcenter_z,threshold=0.5, constant=-1)
